...
export default function App(){
  ...
  return <><OpeningSplash/><BetaWelcome go={go}/><div className={`app-shell ${page === 'home' ? 'home-shell-exact' : ''}`}>{page !== 'home' && <Sidebar page={page} go={go}/>}<main className={`main main-deep ${page === 'home' ? 'main-home main-home-exact' : ''}`}>{page !== 'home' && <Topbar go={go}/>} {content}</main><MobileNav page={page} go={go}/><ShareFeedback/>{page !== 'home' && <KizashiKun page={page} go={go}/>}</div></>;
}
