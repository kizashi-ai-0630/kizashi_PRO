...
export default function KizashiKun({ page, go }) {
  ...
  const onPointerMove = (event) => {
    ...
    const x = Math.max(8, Math.min(window.innerWidth - 92, event.clientX - drag.current.dx));
    const y = Math.max(72, Math.min(window.innerHeight - 120, event.clientY - drag.current.dy));
    setPosition({ x, y });
  };
  const onPointerUp = () => {
    if (!drag.current) return;
  ...
  return <div className="kizashi-kun-wrap" style={style}>
    ...
    >
      <span className="kizashi-hair">〰</span>
      <span className="kizashi-face">◕‿◕</span>
      <span className="kizashi-body">K</span>
      <span className="kizashi-wave">🌊</span>
    </button>
  </div>;
}
